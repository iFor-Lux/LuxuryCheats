...
`;
        ...
        function snoise(v) {
            ...
            const s = (v[0] + v[1]) * F2;
            const i = [Math.floor(v[0] + s), Math.floor(v[1] + s)];
            const G2 = (3 - Math.sqrt(3)) / 6;
            const t = (i[0] + i[1]) * G2;
            const p = [v[0] - i[0] + t, v[1] - i[1] + t];
            
            const [p0, p1] = p[0] > p[1] ? [0, 1] : [1, 0];
            const p2 = [p[0] - p0 + G2, p[1] - p1 + G2];
            const p3 = [p[0] - 1 + 2 * G2, p[1] - 1 + 2 * G2];
            
            return 0.5; // Simplified for now
        }

        function parseLogoImage(img) {
        ...
        // Initialize WebGL and create metallic effect
        async function init() {
            const canvas = document.getElementById('canvas');
            
            // Intentar WebGL2 primero con optimizaciones de rendimiento
            ...
            // Fallback a WebGL si WebGL2 no está disponible
            if (!gl) {
                console.log('[WebGL] WebGL2 not available, falling back to WebGL...');
            ...
            // Fallback final a experimental-webgl
            if (!gl) {
                console.log('[WebGL] WebGL not available, trying experimental-webgl...');
            ...
            img.onload = async () => {
                try {
                    ...
                    function render(currentTime) {
                        ...
                        totalTime += deltaTime * defaultParams.speed;
                        gl.uniform1f(uniforms.u_time, totalTime * 1);

                        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
                        requestAnimationFrame(render);
                    }

                    // Notificar a Android que WebGL está completamente listo (textura cargada, render iniciado)
                    if (window.AndroidInterface && window.AndroidInterface.onWebGLReady) {
                        window.AndroidInterface.onWebGLReady();
                    }
            ...
            img.src = "file:///android_asset/" + url;
        }
